...
from rl_debt_collection.abt import (
    run,
...
from rl_debt_collection.dictionaries import Status
from rl_debt_collection.tables_types import (
    ClientsRow,
    AccountsRow,
    TransactionsRow,
...
def test_make_summary_abt(benchmark):
    ...
    summary_abt_df = benchmark(
        make_summary_abt, highest_period, abt_path, np.random.default_rng()
    )
...
def test_simulate_next_year_for_clients(benchmark):
    ...
    new_clients_df = benchmark(
        simulate_next_year_for_clients, clients_df, np.random.default_rng()
    )
