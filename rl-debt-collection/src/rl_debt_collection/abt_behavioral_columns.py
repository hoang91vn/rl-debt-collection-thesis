...
def make_abt_base(
    ...
    active_aids = (
        cast(pd.Series, transactions_df.loc[period, "status"])
        .loc[lambda df: df == Status.A]
        .index.to_list()
    )
    ...
    abt_base: pd.DataFrame = abt_base_tmp.loc[
        abt_base_tmp["period"] == period, relevant_columns
    ].set_index("aid")

    return abt_base


def make_summary_abt(
    period: int, data_path: str, generator: np.random.Generator
) -> pd.DataFrame:
    max_length: int = 12
    ...
    abt_base_current_period: pd.DataFrame = pd.read_csv(
        ...
        dtype=get_type(AbtBaseRow),  # type: ignore
    )
    ...
    data_output["score"] = (
        data_output["score"] + generator.normal(0, 1, len(data_output)) / 2
    )
    data_output["scorem"] = (
        data_output["scorem"] + generator.normal(0, 1, len(data_output)) / 2
    )
