from typing import Any, Dict, Literal, TypedDict, cast, Self
import pandas as pd
import numpy as np
...
class Account:
    ...
    def generate_account(
        client: Client, app_date: str, number: int, generator: np.random.Generator
    ) -> "Account":
        app_date_dt: datetime = datetime.strptime(app_date, "%Y-%m-%d")
        # period has format YYYYMM
        period: str = app_date_dt.strftime("%Y%m")
        pr: float = (
            0.01 + (1.5 + np.sin(1 * math.pi * 1 / 1) + generator.random() / 5) / 8
        ) / 0.36
        aid: str = (
            "ins"
            + app_date_dt.strftime("%Y%m%d")
            + str(client.cid).zfill(4)
            + str(number)
        )
